...
using vlang::format::FormatStyle;
...
tooling::Replacements reformat(const FormatStyle &Style, StringRef Code,
  ...
  const vlang::FileEntry *Entry =
      Files.getVirtualFile(FileName, Buf->getBufferSize(), 0);
  SourceMgr.overrideFileContents(Entry, Buf);
  FileID ID =
      SourceMgr.createFileID(Entry, SourceLocation(), vlang::SrcMgr::C_User);
...
const char *StyleOptionHelpDescription =
    ...
    ".vlang-format file located in one of the parent\n"
...
FormatStyle getStyle(StringRef StyleName, StringRef FileName,
  ...
  // Look for .vlang-format/_vlang-format file in the file's parent directories.
  ...
  for (StringRef Directory = Path; !Directory.empty();
    ...
    llvm::sys::path::append(ConfigFile, ".vlang-format");
    ...
    if (!IsFile) {
      // Try _vlang-format too, since dotfiles are not commonly used on Windows.
      ConfigFile = Directory;
      llvm::sys::path::append(ConfigFile, "_vlang-format");
      DEBUG(llvm::dbgs() << "Trying " << ConfigFile << "...\n");
      llvm::sys::fs::is_regular_file(Twine(ConfigFile), IsFile);
    }
  ...
  llvm::errs() << "Can't find usable .vlang-format, using " << FallbackStyle
               << " style\n";
  if (!UnsuitableConfigFiles.empty()) {
